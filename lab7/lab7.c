#include<stdio.h>
void main(){
	int ch=0,n,i,at[10],bt[10],rt[10],time_quantum,remain=0,time,count,flag=0,wait_time=0,turnaround_time=0,endTime,smallest,j,sum_wait=0,sum_turnaround=0;
	...
	switch(ch){
		case 1:printf("Enter Time Quantum:\t");
		       ...
		       for(time=0,count=0;remain!=0;){
		       	if(rt[count]>0 && rt[count]<=time_quantum){
				time+=rt[count];
				rt[count]=0;
				flag=1;
			}
